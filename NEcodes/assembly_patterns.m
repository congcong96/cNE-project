function AssemblyTemplates = assembly_patterns(SpikeCount, plotopt, opts)
...
if plotopt == 1
    figure;
    hold on;
    evalsort = sort(eigenvalues, 'descend');
    ...
    plot(1:length(eigenvalues), lambda_max * ones(size(eigenvalues)), 'r-');
    title('E-val dist and M-P boundary');

    plot(evalnum, lambda_min * ones(size(evalnum)), 'r-');
    hold off

    xlim([0 length(evalsort)+1])
...
% figure;
% hold on
% eigtot = sum(eigenvalues);
% evalpercs = cumsum(flipud(evalsort)/eigtot);
% plot(0:length(eigenvalues),[0;evalpercs])
% title('Ratio of variance explained by significant PCs')
% line([0 NumberOfAssemblies],[evalpercs(NumberOfAssemblies) ...
%     evalpercs(NumberOfAssemblies)],'Color','r','LineStyle','--')
% line([NumberOfAssemblies NumberOfAssemblies],[0 evalpercs(NumberOfAssemblies)],...
%     'Color','r','LineStyle','--')
% hold off
