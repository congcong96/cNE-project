function ne_batch_plot_subset_statistics_wo_subsampling(nefiles, varargin)
...
fn_suffix = {'ptd','moransI','reliability_idx'};
...
for i = 1:length(nefiles)    
    ...
    load(nefiles{i}, 'exp_site_nedata', 'nosample_subsetstats')
    ...
    if sum(idx) > 0
        ...
        for j = 1:length(fn_suffix)
            ...
            for k = 1:length(fn_prefix)
                ...
                statstruct(i).([fn_prefix{k} '_' fn_suffix{j} '_zscore']) = [nosample_subsetstats(idx).([fn_prefix{k} '_' fn_suffix{j} '_zscore'])];
