function [autocorrs, crosscorrs] = ne_compare_ne_sta(exp_site_nedata, spktrain)
...
%     NEactUS(i,:) = interp(NEact(i,:), 10);
% end
NEactproj = assembly_activity(ICweights, spktrain);
NEacthalf1 = NEactproj(:,1:floor(size(NEactproj,2)/2));
NEacthalf2 = NEactproj(:,floor(size(NEactproj,2)/2)+1:end);
...
stahalf1 = quick_calc_sta(stimhalf1, NEacthalf1, nlags);
stahalf2 = quick_calc_sta(stimhalf2, NEacthalf2, nlags);
